...
static void (*old_handler[7])(int);
static void (*winch_callback)(void) = NULL;
...
void demo_init(void)
{
	...
	old_handler[6] = signal(SIGWINCH, sigwinch);
	term_setup();
}
...
void demo_exit(void)
{
	...
	signal(SIGWINCH, old_handler[6]);
}

void demo_winch_set(void (*fp)(void))
{
	winch_callback = fp;
}

void demo_winch_unset(void)
{
	winch_callback = NULL;
}
